...
EvaluateOr::EvaluateOr(){
    setOperationType(6);
}

int EvaluateOr::evaluateConditionImple(FactBase* a_Fact, picojson::value* a_Value, ResultInfo* a_ResultInfo){
    ...
    for(int l_count = 0; l_count < m_EvaluateObjectCount; l_count++){
        ...
        if(l_ResultInfo[l_count]->getResultType() == ResultInfo_Bool){
            if(l_ResultInfo[l_count]->getBoolResult() == true){
